...
dh = DataHelper(None, None, FLAGS.test_data_path, FLAGS.map_file_path, FLAGS.batch_size, None,  FLAGS.label2id_file, FLAGS.vocab2id_file, False)
...
with tf.Session(config=config) as sess:
    ...
    qs_y_raw_out = sess.run(qs_y_raw, feed_dict={input_y:np.array(input_y_value, dtype=np.int32), length_y:np.array(length_y_value, dtype=np.int32), keep_prob:1.0})
    ...
    for _, test_batch_q in enumerate(test_batches):
        ...
        test_prob = sess.run(prob_pre, feed_dict={input_x:np.array(input_x_value, dtype=np.int32), length_x:np.array(length_x_value, dtype=np.int32), keep_prob:1.0})  #b*sb
