...
"""
a lstm + dssm implementation for short text matching using tensroflow library
...
class Dssm(object):
    def __init__(self, num_lstm_units, batch_size, negtive_size, SOFTMAX_R, learning_rate, vocab_size, embedding_size=100, use_same_cell=False):
        ...
        with tf.name_scope("keep_prob"):
            self.lstm_fw_cell = rnn.DropoutWrapper(self.lstm_bw_cell, input_keep_prob=self.keep_prob, output_keep_prob=self.keep_prob)
            self.lstm_bw_cell = rnn.DropoutWrapper(self.lstm_bw_cell, input_keep_prob=self.keep_prob, output_keep_prob=self.keep_prob)

        with tf.device('/cpu:0'), tf.name_scope("embedding"):
            #one_gram
            self.vocab = tf.get_variable('w', [vocab_size, embedding_size])
            self.lstm_input_embedding_x = tf.nn.embedding_lookup(self.vocab, self.input_x)  #[batch_size, seq_len, embedding_size]
            self.lstm_input_embedding_y = tf.nn.embedding_lookup(self.vocab, self.input_y)  #[batch_size, seq_len, embedding_size]

        with tf.name_scope('representation'):
            self.states_x = tf.nn.bidirectional_dynamic_rnn(self.lstm_fw_cell, self.lstm_bw_cell, self.lstm_input_embedding_x, self.length_x,
                                                            dtype=tf.float32)[1]
        ...
        with tf.name_scope('rotate'):
            ...
            for i in range(negtive_size):
                ...
                self.qs_y = tf.concat(axis=0, values=[self.qs_y, rand_qs_y1, rand_qs_y2])  #[batch_size*(negtive_size+1), num_lstm_units*2]

        with tf.name_scope('sim'):
            # cosine similarity
            q_norm = tf.tile(tf.sqrt(tf.reduce_sum(tf.square(self.q_y_raw), 1, True)), [negtive_size + 1, 1])  #[(negtive_size + 1) * batch_size, 1]
            qs_norm = tf.sqrt(tf.reduce_sum(tf.square(self.qs_y), 1, True))  #[batch_size*(negtive_size+1), 1]
            prod = tf.reduce_sum(tf.multiply(tf.tile(self.q_y_raw, [negtive_size + 1, 1]), self.qs_y), 1, True)  #[batch_size*(negtive_size + 1), 1]
            ...
            self.cos_sim = tf.transpose(tf.reshape(tf.transpose(sim_raw), [negtive_size + 1, batch_size])) * SOFTMAX_R  #[batch_size, negtive_size + 1]
