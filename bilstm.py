    ...
    def __init__(self, FLAGS):
        ...
        with tf.variable_scope("embedding", initializer=tf.orthogonal_initializer()):
            with tf.device('/cpu:0'):
                ...
                inputs = tf.split(embedded, FLAGS.seq_length, 1)  # [[batch_size, 1, embedding_size], [batch_size, 1, embedding_size], number is seq_length]
                inputs = [tf.squeeze(input_, [1]) for input_ in inputs]  # [[batch_size, embedding_size], [batch_size, embedding_size], number is seq_length]
        ...
        with tf.variable_scope("encoder", initializer=tf.orthogonal_initializer()):
            ...
            lstm_fw_cell_stack = rnn.DropoutWrapper(lstm_fw_cell_stack, input_keep_prob=self.dropout_keep_prob, output_keep_prob=self.dropout_keep_prob)
            lstm_bw_cell_stack = rnn.DropoutWrapper(lstm_bw_cell_stack, input_keep_prob=self.dropout_keep_prob, output_keep_prob=self.dropout_keep_prob)
            self.outputs, self.fw_st, self.bw_st = rnn.static_bidirectional_rnn(lstm_fw_cell_stack, lstm_bw_cell_stack, inputs, sequence_length=self.x_len, dtype=tf.float32) #multi-layer
            #only use the last layer
            last_layer_no = FLAGS.lstm_layers - 1
            self.states = tf.concat([self.fw_st[last_layer_no].h, self.bw_st[last_layer_no].h], 1)  #[batchsize, (num_units * 2)]

        attention_size = 2 * FLAGS.num_units
        with tf.variable_scope('attention'):
            attention_w = tf.Variable(tf.truncated_normal([2 * FLAGS.num_units, attention_size], stddev=0.1), name='attention_w')  #[num_units * 2, num_units * 2]
            ...
            u_w = tf.Variable(tf.truncated_normal([attention_size, 1], stddev=0.1), name='attention_uw')  #[num_units * 2, 1]
            attn_z = []
            for index in range(FLAGS.seq_length):
            ...
            alpha_trans = tf.reshape(tf.transpose(alpha, [1, 0]), [FLAGS.seq_length, -1, 1])  #[seq_length, batchsize, 1]
